...
- [ ] Every submission or confirmation transaction that writes a pending line, batch balance, or movement must acquire the Task 2 catalog resource locks before its first such write: lock all affected locations with `private.lock_warehouse_location_resource(uuid)` in ascending UUID order, then all affected variants with `private.lock_warehouse_variant_resource(uuid)` in ascending UUID order. After all locks are held, re-read and require every referenced location and variant to be active; never rely on a pre-lock browser value or an earlier snapshot. Add dual-connection proofs that a concurrent location/variant deactivation either waits and is refused or commits before the workflow write, which must then fail closed.
