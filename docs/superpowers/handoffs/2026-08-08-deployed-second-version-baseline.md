...
After the exact import, four recovered static test fixtures were intentionally corrected. `src/services/projectDocumentSchema.test.js` now asserts the recovered prototype migration's actual equivalent security semantics instead of stale DDL names. Three contract-revenue tests now assert the recovered service-authoritative state and permission-gated callbacks instead of the superseded local-persistence wiring. Runtime source, service code, and the recovered migration remain byte-identical. Therefore 279/283 manifest paths remain byte-identical to recovered source and four paths have the documented fixture-only deltas below.

### Intentional fixture-only deviations after import

| Fixture path | Recovered SHA-1 | Current SHA-1 | Corrected mismatch |
|---|---|---|---|
...
| `src/auth/AuthGate.jsx` | `99e0f6218958920bbb404fa0b6ea4922b2a49b00` | exact |
