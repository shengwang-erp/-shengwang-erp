...
`202608090001`、`202608100002`、`202608100003`、`202608110001`–`003`、
`202608120001`、`202608120002`；
...
部门考勤和公司人员工资的生产发布采用固定顺序：先确认可恢复 Supabase 备份并记录 migration
versions；再应用 `202608120001` 并验证 v1/v2 attendance RPC；随后应用 `202608120002` 并执行
只读会计/工资对象、约束和聚合计数探针；之后才部署已验证的 Vercel production build。发布后
必须分别 smoke-test 工程、通用、免打卡、会计和工资禁止账号，最后确认
`https://shengwang-erp.vercel.app/` main alias。v1 RPC 在过渡期保留，不随前端发布删除。

只读探针及完整发布记录格式见
[today-attendance-operations.md](./today-attendance-operations.md) 和
[attendance-accounting-operations.md](./attendance-accounting-operations.md)。这些探针不得选择员工
姓名、工资金额、坐标、说明或照片字段；任何 migration 缺失、RPC 不存在、关键约束未验证或聚合
模式超出 `project/general/exempt` 都必须停止发布。
