...
- `tool-responsibility:`：项目工具丢失按工具价值、损坏按维修费用计入；员工赔偿另行记录，不静默抵减项目毛成本；
- `operating:`：启用项目分摊并绑定项目的经营费用；
- `legacy-manual:`：历史有效手工项目成本；
- `manual:`：会计通过明细账新增的正数或负数费用。

已删除、已取消、已作废、项目无效或金额/关键字段不合规的来源不作为正常零值处理。已由仓库冻结批次成本代表的采购，只计算仓库成本，不再重复计算采购总额。
...
- 已有拆分历史的来源需要更正金额时，应追加手工调整/冲销并重新拆分，不能让旧拆分快照与新金额不一致。
