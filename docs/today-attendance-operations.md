...
6. 验证 Vercel main production alias 指向本次已验证部署。v1 RPC 只作为数据库/API 签名兼容和
   `project` 账号分批上线边界保留，不作为通用或免打卡数据出现后的旧前端完整回退方案。

该顺序是兼容性边界：数据库迁移先为历史场次回填 `project` 模式并同时保留 v1/v2，前端随后
切换到 v2。`202608120002` 必须在 `202608120001` 之后，Vercel 必须在两者及只读探针之后。
不得在同一发布中删除或收回 v1 RPC；只有单独评审的后续清理迁移才能结束过渡期。发布必须
使用维护窗口并以 roll-forward 为默认恢复方式：旧客户端只能读取严格的 project 投影，且
general/exempt 账号调用旧项目打卡会在服务器端失败关闭。出现前端故障时应保持新数据库不动，
暂停受影响入口，修复并重新发布新前端；不得把生产 alias 回退到会解析 general 数据的旧版本。

精确回滚边界：在尚未产生任何 general 场次、政策历史或新会计快照前，可按部署记录评估恢复
整库备份；一旦已产生上述事实，不支持只回退前端或只撤销 `202608120001/002`。此时必须进入
维护状态并 roll-forward，只有经审计确认的整库时间点恢复才可作为灾难恢复方案。陈旧的旧版
general/exempt 客户端必须继续失败关闭，不能临时放宽 v1 mutation 门禁。
