...
test('App wires the Miraisya panel to stable billing and authoritative cost services', () => {
  assert.match(appSource, /import MiraisyaProjectPanel from ['"]\.\/features\/miraisya\/MiraisyaProjectPanel/)
  assert.match(appSource, /import \{ miraisyaBillingService \} from ['"]\.\/services\/miraisyaBillingService/)
  assert.match(appSource, /miraisyaProjectId/)
  assert.match(appSource, /<MiraisyaProjectPanel/)
  assert.match(appSource, /billingService=\{miraisyaBillingService\}/)
  assert.match(appSource, /costLedgerService=\{activeProjectCostLedgerService\}/)
  assert.match(appSource, /onOpenMiraisyaProject=/)
  assert.doesNotMatch(appSource, /projects=\{projects\.filter\([^)]*projectType/)
})
