...
const SNAPSHOT = {
  rows: [
    COMPLETE_ROW,
    {
      ...
      operator: '', adjusted: false, version: 1, allocations: [], auditEvents: [],
    },
    {
...
test('paginates immutable row collections without changing the source rows', () => {
  ...
  assert.deepEqual(paginateLedgerRows(rows, 2, 2).map(({ sourceKey }) => sourceKey), ['labor:L-1'])
...
test('summarizes ledger rows in fixed category order using four-decimal units', () => {
  const summary = summarizeLedgerRows([
    { ...COMPLETE_ROW, category: '其他费用', originalAmount: 0.3, effectiveAmount: 0.2, adjustmentAmount: -0.1 },
    { ...COMPLETE_ROW, category: '材料费', originalAmount: 0, effectiveAmount: 0.1, adjustmentAmount: 0.1 },
    { ...COMPLETE_ROW, category: '人工费', originalAmount: 1.005, effectiveAmount: 1.005, adjustmentAmount: 0, adjusted: false },
  ])
...
test('percentage allocations retain fixed-point precision at the safe-unit boundary', () => {
  assert.deepEqual(buildAllocationAmounts(900719925474.0991, [
    { projectId: 'P1', mode: 'percent', value: 50 },
    { projectId: 'P2', mode: 'percent', value: 50 },
  ]), [
    { projectId: 'P1', amount: 450359962737.0496 },
    { projectId: 'P2', amount: 450359962737.0495 },
  ])
})
