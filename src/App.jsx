...
// 以后接企业微信登录后，currentUser 要从登录身份读取。
const currentUser = {
  employeeId: 'E001',
  name: '老板',
  position: '老板',
...
function normalizeEmployee(employee) {
  ...
  return {
    ...
    baseSalary: toAmount(employee.baseSalary),
    dailySalary: toAmount(employee.dailySalary),
    hourlyWage: toAmount(employee.hourlyWage),
...
function calculateLaborCost(workType, dailySalary, hourlyWage, workHours, manualCost) {
...
function hasTimeConflict(record, allRecords) {
  ...
  return allRecords.some((item) => {
    const current = normalizeLaborRecord(item)
    if (
...
function getLaborExceptions(records) {
  ...
  return normalizedRecords.flatMap((record) => {
    ...
    if (hasTimeConflict(record, normalizedRecords)) {
      issues.push('同一时间段存在多条出工记录')
    }
...
function HomePage({ projects, employees, records, accountingRecords, onOpenView }) {
  ...
  const activeEmployees = employees.filter((employee) => employee.employmentStatus === '在职').length
  ...
  const monthlyCostTotal =
    accountingRecords.salary
      .filter((record) => record.salaryMonth === currentMonthValue())
      .reduce((total, record) => total + toAmount(record.netSalary), 0) +
      ...
      {isFormOpen && (
        <form className="form-panel" onSubmit={handleSubmit}>
          <div className="form-grid">
            ...
            <Field label="基本工资" type="number" value={form.baseSalary} onChange={(value) => setForm({ ...form, baseSalary: value })} />
            <Field label="日工资" type="number" value={form.dailySalary} onChange={(value) => setForm({ ...form, dailySalary: value })} />
          ...
          filteredEmployees.map((employee) => (
            <article className="record-card" key={employee.employeeId}>
              ...
              <dl className="detail-list compact">
                ...
                <div><dt>日工资</dt><dd>{formatYen(employee.dailySalary)}</dd></div>
                <div><dt>时薪</dt><dd>{formatYen(employee.hourlyWage)}</dd></div>
              </dl>
              <EmployeeOwnedTools
                employee={employee}
...
function LaborPage({ projects, employees, records, setRecords, onBack }) {
  ...
  const handleSubmit = (event) => {
    ...
    nextRecords.forEach((record) => {
      if (hasTimeConflict(record, records)) {
        window.alert(`员工${record.employeeName}在该时间段已有出工记录，请确认是否重复录入。`)
      }
    ...
    <PageShell title="人工记录" subtitle="业务记录 · 引用人员管理" onBack={onBack}>
      ...
      <form className="form-panel" onSubmit={handleSubmit}>
          ...
          <dl className="detail-list compact employee-snapshot">
            {previewEmployees.map((employee) => (
              <div key={employee.employeeId}>
                <dt>{employee.name}</dt>
                <dd>{employee.department}｜{employee.position}｜{employee.level}｜日工资 {formatYen(employee.dailySalary)}</dd>
              </div>
            ))}
          </dl>
        )}
        <div className="form-grid">
          ...
          {form.workType === '其他' && (
            <Field label="手动人工成本" type="number" value={form.laborCost} onChange={(value) => setForm({ ...form, laborCost: value })} />
          )}
          <ReadOnlyField
            label={editingId ? '人工成本' : '本次人工成本合计'}
            value={formatYen(
              editingId
                ? laborCost
  ...
  return (
    <>
      ...
      <form className="form-panel" onSubmit={handleSubmit}>
        ...
        {selectedEmployee && (
          <dl className="detail-list compact employee-snapshot">
            <div><dt>基本工资标准</dt><dd>{formatYen(selectedEmployee.baseSalary)}</dd></div>
            <div><dt>日工资标准</dt><dd>{formatYen(selectedEmployee.dailySalary)}</dd></div>
            <div><dt>时薪标准</dt><dd>{formatYen(selectedEmployee.hourlyWage)}</dd></div>
          </dl>
        )}
        <div className="form-grid">
          <Field
...
function MonthlySummarySection({
  ...
  const [monthFilter, setMonthFilter] = useState(currentMonthValue())
  const totalSalary = salaryRecords
    .filter((record) => record.salaryMonth === monthFilter)
    .reduce((total, record) => total + toAmount(record.netSalary), 0)
  ...
  const totalVehicleCost =
    ...
    monthlyVehicleIssues.reduce((total, record) => total + toAmount(record.repairCost), 0)
  const totalCost =
    totalSalary + totalProjectCost + totalOperatingExpense + totalPurchaseCost + totalVehicleCost
  ...
  return (
    <>
      ...
      <div className="empty-state cost-note">采购支出、绑定项目的车辆费用已计入项目成本统计时，请避免再手工重复录入同一笔费用。</div>
    </>
  )
}
  ...
  return (
    <>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card money"><strong>{formatYen(totalLaborCost)}</strong><span>人工成本合计</span></div>
                  ...
                  <article className="labor-movement-card" key={item.id}>
                    <div className="record-header">
                      <div>
                        ...
                        <span>{item.record.workDate}｜{item.record.employeeName || '未绑定员工'}｜{item.record.projectName || item.record.workLocationType}</span>
                      </div>
                    </div>
                  </article>
                ))}
                  ...
                  <div className="timeline-list">
                    {employee.records.map((record) => (
                      <article className="timeline-card" key={record.laborRecordId}>
                        ...
                        <span>人工成本：{formatYen(record.laborCost)}</span>
                      </article>
                    ))}
                  </div>
                </details>
            ...
            projectStats.map((project) => (
              <article className="labor-movement-card" key={project.key}>
                ...
                <dl className="detail-list compact">
                  ...
                  <div><dt>人工成本合计</dt><dd>{formatYen(project.totalLaborCost)}</dd></div>
                </dl>
                <details className="batch-detail">
...
function DashboardPage({
    ...
    return [
      ...
      { label: '退回记录数量', value: byProject(records.stockReturn) },
      {
        ...
        extra: [
          ...
          `本月人工成本 ${formatYen(currentMonthLaborRecords.reduce((total, record) => total + toAmount(record.laborCost), 0))}`,
  ...
  const personnelStats = useMemo(() => {
    ...
    const monthlySalary = salaryRecords
      .filter((record) => record.salaryMonth === currentMonth)
      .reduce((total, record) => total + toAmount(record.netSalary), 0)
    const monthlyLaborCost = records.labor
      .filter((record) => monthFromDate(record.workDate || record.date) === currentMonth)
      .reduce((total, record) => total + toAmount(record.laborCost), 0)
    ...
    return [
      ...
      { label: '本月工资合计', value: formatYen(monthlySalary), tone: 'money' },
      { label: '本月人工成本合计', value: formatYen(monthlyLaborCost), tone: 'money' },
      { label: '最高权限人数', value: employees.filter((employee) => isSuperAdmin(employee)).length },
      { label: '自定义权限人数', value: employees.filter((employee) => employee.role === 'custom').length },
      { label: '普通员工人数', value: employees.filter((employee) => employee.role === 'employee').length },
      { label: '老板人数', value: employees.filter((employee) => employee.position === '老板').length },
      { label: '操作员人数', value: employees.filter((employee) => employee.position === '操作员').length },
      { label: '设计部部长人数', value: employees.filter((employee) => employee.position === '设计部部长').length },
    ]
  }, [employees, salaryRecords, records.labor])

  const departmentStats = countBy(employees, 'department')
  const positionStats = countBy(employees, 'position')
  const levelStats = countBy(employees, 'level')
  const visaWarnings = employees
...
function EmployeeSelect({
  ...
  const availableEmployees = employees.filter(
    (employee) => showResigned || employee.employmentStatus !== '离职',
  )
...
function EmployeeMultiSelect({ employees, selectedIds, onChange }) {
  ...
  const selectedEmployees = employees.filter((employee) => selectedIds.includes(employee.employeeId))
      ...
      {employees.length === 0 ? (
        ...
        <>
          ...
          <div className="employee-option-grid">
            {availableEmployees.map((employee) => (
              <label className="employee-option" key={employee.employeeId}>
                <input
                  type="checkbox"
